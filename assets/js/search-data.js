// get the ninja-keys element
const ninja = document.querySelector('ninja-keys');

// add the home and posts menu items
ninja.data = [{
    id: "nav-about",
    title: "about",
    section: "Navigation",
    handler: () => {
      window.location.href = "/";
    },
  },{id: "nav-publications",
          title: "publications",
          description: "publications by categories in reversed chronological order. generated by jekyll-scholar.",
          section: "Navigation",
          handler: () => {
            window.location.href = "/publications/";
          },
        },{id: "nav-projects",
          title: "projects",
          description: "A growing collection of your cool projects.",
          section: "Navigation",
          handler: () => {
            window.location.href = "/projects/";
          },
        },{id: "nav-cv",
          title: "cv",
          description: "Curriculum vitae of Junhong Zhang — English | 中文版.",
          section: "Navigation",
          handler: () => {
            window.location.href = "/cv/";
          },
        },{id: "news-my-first-author-paper-npsvc-a-representation-learning-framework-for-nonparallel-classifiers-has-been-accepted-by-ieee-transactions-on-neural-networks-and-learning-systems-tnnls-code-is-available-on-github",
          title: 'My first-author paper “NPSVC++: A Representation Learning Framework for Nonparallel Classifiers” has been...',
          description: "",
          section: "News",},{id: "news-two-papers-i-co-authored-have-been-accepted-as-posters-to-acm-multimedia-2026-nsmnet-stabilizing-linear-state-space-memory-for-breast-ultrasound-video-segmentation-and-a-large-scale-multi-modal-benchmark-and-robust-text-free-inference-for-breast-ultrasound-video-segmentation",
          title: 'Two papers I co-authored have been accepted as posters to ACM Multimedia 2026:...',
          description: "",
          section: "News",},{id: "news-i-have-started-my-visiting-research-position-at-the-department-of-mathematics-hong-kong-baptist-university-kowloon-campus",
          title: 'I have started my visiting research position at the Department of Mathematics, Hong...',
          description: "",
          section: "News",},{id: "projects-joker",
          title: 'Joker',
          description: "Joint optimization framework for lightweight kernel machines (ICML 2025)",
          section: "Projects",handler: () => {
              window.location.href = "/projects/1_joker/";
            },},{id: "projects-npsvc",
          title: 'NPSVC++',
          description: "A representation learning framework for nonparallel classifiers (IEEE TNNLS 2026)",
          section: "Projects",handler: () => {
              window.location.href = "/projects/2_npsvcpp/";
            },},{id: "projects-odsvm",
          title: 'ODSVM',
          description: "Learning the optimal discriminant SVM with feature extraction (IEEE TPAMI 2025)",
          section: "Projects",handler: () => {
              window.location.href = "/projects/3_odsvm/";
            },},{id: "projects-lightwsvm",
          title: 'LighTwSVM',
          description: "Efficient linear nonparallel classifier for millions of data (Pattern Recognition 2025)",
          section: "Projects",handler: () => {
              window.location.href = "/projects/4_lightwsvm/";
            },},{id: "projects-rmtbsvm",
          title: 'RMTBSVM',
          description: "Robust twin bounded SVM with manifold regularization (IEEE TCYB 2023)",
          section: "Projects",handler: () => {
              window.location.href = "/projects/5_rmtbsvm/";
            },},{id: "projects-svm-intro",
          title: 'SVM-Intro',
          description: "An annual beginner-friendly course on support vector machines",
          section: "Projects",handler: () => {
              window.location.href = "/projects/6_svm_intro/";
            },},{
        id: 'social-cv',
        title: 'CV',
        section: 'Socials',
        handler: () => {
          window.open("/assets/pdf/resume.pdf", "_blank");
        },
      },{
        id: 'social-email',
        title: 'email',
        section: 'Socials',
        handler: () => {
          window.open("mailto:%61%70%70%6C%65_%7A%6A%68@%31%36%33.%63%6F%6D", "_blank");
        },
      },{
        id: 'social-github',
        title: 'GitHub',
        section: 'Socials',
        handler: () => {
          window.open("https://github.com/Apple-Zhang", "_blank");
        },
      },{
        id: 'social-rss',
        title: 'RSS Feed',
        section: 'Socials',
        handler: () => {
          window.open("/feed.xml", "_blank");
        },
      },{
      id: 'light-theme',
      title: 'Change theme to light',
      description: 'Change the theme of the site to Light',
      section: 'Theme',
      handler: () => {
        setThemeSetting("light");
      },
    },
    {
      id: 'dark-theme',
      title: 'Change theme to dark',
      description: 'Change the theme of the site to Dark',
      section: 'Theme',
      handler: () => {
        setThemeSetting("dark");
      },
    },
    {
      id: 'system-theme',
      title: 'Use system default theme',
      description: 'Change the theme of the site to System Default',
      section: 'Theme',
      handler: () => {
        setThemeSetting("system");
      },
    },];
